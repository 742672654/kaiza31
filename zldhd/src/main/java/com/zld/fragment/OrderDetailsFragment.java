...
//		setShowBillButton();
	...
	private void initView(View rootView) {
        iv_car_image = rootView.findViewById(R.id.iv_car_image);
        et_car_num = rootView.findViewById(R.id.et_car_num);
//		btn_bigsmall_switch = (Button) rootView.findViewById(R.id.btn_bigsmall_switch);
        btn_car_type = rootView.findViewById(R.id.btn_car_type);
        tv_account_type = rootView.findViewById(R.id.tv_account_type);
        tv_entrance_time = rootView.findViewById(R.id.tv_entrance_time);
        tv_park_duration = rootView.findViewById(R.id.tv_park_duration);
        btn_clear_order = rootView.findViewById(R.id.btn_clear_order);
...
//	}
	...
	private void setSwitchCarType(CarNumberOrder order){
		...
		if (order != null){
			...
			for (int i = 0; i < count; i++) {
				String name = AppInfo.getInstance().getAllCarTypes().get(i).getValue_no();
				if (name.equals(carType)) {
					carTypeName = AppInfo.getInstance().getAllCarTypes().get(i).getValue_name();
			...
			if (carTypeName == null){
				carTypeName = AppInfo.getInstance().getAllCarTypes().get(0).getValue_name();
				btn_car_type.setText(carTypeName);
			}
	...
	private void modifyOrder() {
		...
		if(!AppInfo.getInstance().getIsLocalServer(activity)){//是本地服务器则没有平板本地化的感念
			...
			boolean param = SharedPreferencesUtils.getParam(
					activity.getApplicationContext(), "nettype", "isLocal", false);
            Log.e("isLocal", "OrderDetailsFragment6 modifyOrder get isLocal " + param);
//			if(param||AppInfo.getInstance().getIssuplocal().equals(Constant.sOne)){
////				doModifyOrderTimeOut();
//				return;
//			}
