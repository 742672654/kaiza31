	...
	private void saveResult(Bitmap bitmap, byte[] carPlate, int height, int width, int left, int top) {
	 ...
	 class MyThread extends Thread{//继承Thread类
		...
		public void run(){
			...
			Log.i("拍照","resType:"+resType+"---nType:"+nType+"---carPlateColor:"+carPlateColor);
			...
			if ( !"_无_".equals(AllcarPlate) &&PARKING_ip_Bean.isType(cameraIp)==2 ){  //cameraIp.equals("192.168.1.198")
				...
				//		case 2:new Led_in_Thread(PARKING_ip_Bean.typeIP(2),"车位已满无法进入").start();return;
						default:break;
					}
				}catch (Exception e){
					e.printStackTrace();
				...
				try{
					...
					if (object.getInt("reserve")==1){
						//new Led_in_Thread(PARKING_ip_Bean.typeIP(3),AllcarPlate+"提前缴费").start();
						taigan.tai(cameraIp);
						return;
					}else if (object.getInt("reserve")==2){

						//new Led_in_Thread(PARKING_ip_Bean.typeIP(3),AllcarPlate+"欠费"+object.getInt("arrears")+"元").start();
						return;
					}
				}catch (Exception e){
					e.printStackTrace();
	...
	public void yitijiThread(final String cameraIp) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				Log.i("出口摄像头","runYitiji:"+cameraIp);
